...
tabstat thearn_lag if trans_bw60_alt2 & bw60lag==0, by(educ_gp) stats(mean p50) // in table 3, PRE
tabstat thearn if trans_bw60_alt2, by(educ_gp) stats(mean p50) //
tabstat thearn_alt if trans_bw60_alt2 & bw60lag==0, by(educ_gp) stats(mean p50) // I think this is right for POST
...
regress hh_income_raw i.pathway thearn_lag if trans_bw60_alt2==1
regress hh_income_raw i.educ_gp i.pathway thearn_lag if trans_bw60_alt2==1
regress hh_income_raw i.pathway i.race i.educ_gp thearn_lag if trans_bw60_alt2==1
...
/* moving most of this to step a, because that logistically makes sense
